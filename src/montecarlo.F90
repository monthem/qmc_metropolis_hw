...
subroutine gen_metropolis_monte_carlo(nstep, dt, n, A, Z, nuclei, coeffs, alpha, energy, acceptance)
    ...
    double precision, intent(in)                        :: Z(A), nuclei(A, 3)
    double precision, intent(in)                        :: coeffs(A), alpha, energy, acceptance
    ! Local variables
    integer(kind=8)                                     :: step, accepted
    double precision                                    :: electrons(n, 3), Q_old(n, A, 3), Q_new(n, A, 3)
    double precision                                    :: r_old(3), r_new(3), phi_old, phi_new
    double precision                                    :: d_old(3), d_new(3), chi(3), diffusion(3)
    double precision                                    :: v, a_prob, sqrt_dt, P
    double precision                                    :: acceptance_i(n), a_prob
    ...
    do i = 1, n
        ...
        call form_Q(A, n, nuclei, electrons_old, Q_old) ! Not optimal for just one electron update, but not intense either
    ...
    do step = 1, nstep
        ...
        do i = 1, n
           ...
           call gmc_acceptance_probability(electrons_old(i,:), electrons_new(i,:), drift_vectors_old(i,:), drift_vectors_new(i,:), dt, a_prob)
           a_prob = a_prob * P
           ...
           if (v <= a_prob) then
               ...
               orbitals_old(i) = orbitals_old(i)
            end if
        end do
        V_EE = el_el(n, A, Q_new(i,:,:)
        energy = energy + V_EE
    end do
